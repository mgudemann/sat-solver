...
cdcl clist valuation stats fullStats = case (valuation, result) of
            (True, SAT_SOLUTION l)                          -> SAT_SOLUTION (l ++ decodedPureVals)
            (True, SAT_WITH_STATS l a0 a1 a2 a3)            -> SAT_WITH_STATS (l ++ decodedPureVals) a0 a1 a2 a3
            (True, SAT_WITH_FULL_STATS l a0 a1 a2 a3 a4 a5) -> SAT_WITH_FULL_STATS (l ++ decodedPureVals) a0 a1 a2 a3 a4 a5
            (_, UNSAT)                                      -> UNSAT
            (_, UNSAT_WITH_STATS a0 a1)                     -> UNSAT_WITH_STATS a0 a1
            _                                               -> result
            where (reducedTerm, pureVars) = rmPureVars clist
                  decodedPureVals = map decodeTuple pureVars
                  result = _cdcl reducedTerm stats fullStats
...
_cdcl clist stats fullStats
    ...
    | null clist = SAT_SOLUTION [] --[] Map.empty 0
    | otherwise = case result of
                    SAT_SOLUTION _                    -> SAT
                    SAT_WITH_STATS _ _ _ _ _          -> SAT
                    SAT_WITH_FULL_STATS _ _ _ _ _ _ _ -> SAT
                    _                                          -> result
